...
import { saveImageInFirebase, validateQuiz } from "@/utils";
...
const page = () => {
  ...
  const saveQuiz = async () => {
    try {
      ...
      // Before making a request to the backend, validate the quiz data
      ...
      let { coverImageBlob, ...saveRequestData } = pageData;

      saveRequestData = {
        ...saveRequestData,
        questions: saveRequestData.questions.map((question) => {
          if (question.questionImageBlob) {
            const { questionImageBlob, ...cleanedQuestionData } = question;
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/quizzes/create-quiz`,
        {
          method: "POST",
          headers: {
      ...
      if (pageDataCopy.coverImageBlob) {
        const coverImageUrl = await saveImageInFirebase(
          pageDataCopy.coverImageBlob,
          `/users/${uid}/quizzes/${data.quiz_id}/icon.jpg`,
        );
        pageDataCopy = {
          ...pageDataCopy,
      ...
      for (let questionObject of pageData.questions) {
        ...
        const questionImageUrl = await saveImageInFirebase(
          questionObject.questionImageBlob,
          `/users/${uid}/quizzes/${data.question_ids[questionObject.uid]}/questions/${questionObject.id}.jpg`,
        );
        pageDataCopy = {
          ...pageDataCopy,
          questions: pageDataCopy.questions.map((question) =>
      ...
      const updateResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/quizzes/quiz/${quiz_id}/update`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "Application/Json",
