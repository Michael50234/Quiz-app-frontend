...
const page = () => {
  ...
  // Redirect users if they are not the owner
  useEffect(() => {
    ...
    if (pageData.owner.id !== user?.id) {
      ...
      showError("You do not have the permissions to access this page");
    }
  }, [userLoading, loading]);
  ...
  const fetchQuiz = async (tags: Tag[]) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/quizzes/quiz/${quizId}`,
      {
        method: "GET",
        headers: {
  ...
  const saveImageInFirebase = async (
  ...
  const changeChoice = (
    ...
    setPageData((prev) => {
      ...
      return {
        ...prev,
        questions: prev.questions.map((question) => {
          return question.uid != questionUid
            ? question
            : {
                ...question,
                choices: question.choices.map((choice) => {
    ...
    try {
      ...
      updateData = {
        ...
        questions: updateData.questions.map((question) => {
          let { questionImageBlob, question_image_url, ...cleanedQuestion } =
            question;
      ...
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/quizzes/quiz/${quizId}/update`,
        {
          method: "PATCH",
          headers: {
      ...
      if (pageDataCopy.coverImageBlob) {
        pageDataCopy.cover_image_url = await saveImageInFirebase(
          pageDataCopy.coverImageBlob,
          `users/${uid}/quizzes/${quizId}/icon.jpg`,
        );
      }

      // Save question images
      pageDataCopy.questions = await Promise.all(
        pageDataCopy.questions.map(async (question) => {
          if (question.questionImageBlob) {
            const new_image_url = await saveImageInFirebase(
              question.questionImageBlob,
              `users/${uid}/quizzes/${quizId}/questions/${data["question_ids"][question.uid]}.jpg`,
            );
        ...
        <Dialog open={saveDialogOpen} onClose={() => setSaveDialogOpen(false)}>
          ...
          <DialogContent>
            ...
            <DialogActions>
              ...
              <Button sx={{ height: "40px" }} onClick={saveQuiz}>
