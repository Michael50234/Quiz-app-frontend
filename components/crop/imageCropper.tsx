...
const ImageCropper = ({
  ...
  changeImageBlob: (blob: Blob) => void;
  ...
  const onCropComplete = useCallback(
    (
      ...
      croppedPixels: Record<string, number>,
    ): void => {
  ...
  const onSave = async () => {
    try {
      ...
      const croppedImgBlob: Blob = await getCroppedImage(
        image,
        croppedAreaPixels,
      );
  ...
  return (
    ...
    >
      <Box sx={{ display: "flex" }}>
        ...
        <Button
          disabled={!croppedAreaPixels || !image}
          onClick={() => onSave()}
        >
          Save Image
        </Button>
